...
typedef struct pattern {
  ...
  */
  ...
  int outlier_score; // comes from ransac svm (implement later)
} PATTERN;
...
typedef struct latent_var {
  ...
  */
  int displacement;  // the displacement referes to the displacement'th feature of a particular xi with each feature being of 4096 dimentions
} LATENT_VAR;
...
typedef struct struct_learn_parm {
  ...
  int num_features;  // 4096 in our case, this is required as in psi() sample will not be sent , but we still nedd to know the dimension of w 
} STRUCT_LEARN_PARM;
