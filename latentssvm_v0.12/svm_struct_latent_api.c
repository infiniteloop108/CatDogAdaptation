...
*/
  ...
  if(fin != NULL)
  {
	  ...
	  fscanf(fin,"%d",&n);
	  sample.n = n;
	  sample.examples = (EXAMPLE *) malloc(sizeof(EXAMPLE) * (n+1));	//1 indexed (odd)
	  for(int i=1;i<=n;++i)
	  {
		  char fileName[100];
		  int class;
		  ...
		  sample.examples[i].h.box = 0;	//How should we initialize h?
		  ...
		  {
			  fscanf(fin, "%f", &sample.examples[i].x.data[j]);
		  }
		  fclose(data);
	  }
	  fclose(fin);
...
*/
  ...
  sm->sizePsi = sample.examples[0].x.dimension; /* replace with appropriate number */
